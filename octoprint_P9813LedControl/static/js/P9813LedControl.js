/*
 * View model for P9813 LED Control
 *
 * Author: You
 * License: AGPLv3
 */
$(function () {
    function P9813LedControlStatusViewModel (parameters) {
        ...
        function update_light_status (response) {
            ...
            if (response.torch_status) {
                torch_icon.attr(
                    'src',
                    'plugin/P9813LedControl/svg/flashlight.svg'
                )
            } else {
                torch_icon.attr(
                    'src',
        ...
        self.onBeforeBinding = function () {
            OctoPrint.simpleApiGet('P9813LedControl').done(update_light_status)
            self.torch_enabled(
    ...
    OCTOPRINT_VIEWMODELS.push({
        ...
        elements: ['#P9813LedControl_navbar_led_status']
    })

    function P9813ledcontrolViewModel (parameters) {
        var self = this
    }

    OCTOPRINT_VIEWMODELS.push({
        construct: P9813ledcontrolViewModel,
        dependencies: ['settingsViewModel'],
        elements: ['settings_plugin_P9813LedControl']
    })
})
